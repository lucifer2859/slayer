...
if __name__ == '__main__':      
    ...
    for epoch in range(100):
        ...
        for i, (inputs, targets, labels) in enumerate(trainLoader, 0):
            # Move the input and target to correct GPU.
            inputs  = inputs.to(device)  # torch.Size([8, 2312, 1, 1, 300])
            targets = targets.to(device) # torch.Size([8, 10, 1, 1, 1])
            
            # Forward pass of the network.
            outputs = net.forward(inputs) # torch.Size([8, 10, 1, 1, 300])
            
            # Gather the training stats.
            stats.training.correctSamples += torch.sum(snn.predict.getClass(outputs) == labels).data.item()
            stats.training.numSamples     += len(labels)
            
            # Calculate loss.
            loss = error.numSpikes(outputs, targets)
        ...
        for i, (inputs, targets, labels) in enumerate(testLoader, 0):
            inputs  = inputs.to(device)
            targets = targets.to(device) 
            
            outputs = net.forward(inputs)

            stats.testing.correctSamples += torch.sum(snn.predict.getClass(outputs) == labels).data.item()
